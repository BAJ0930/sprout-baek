...
if(dom) {
	...
	strCalendar += "td {font-size:12px; font-family:굴림; text-decoration:none;padding:0px; margin:0px; }";
	...
	strCalendar += ".cnj_close {font-size:8pt;color:#000000;padding:0px; margin:0px;  background-color:#EFEFEF;  border: thin solid #808080;  border-style:solid;cursor:hand;font-weight:bold;height:16px;width:16px;text-align:center;vertical-align:bottom}";
	strCalendar += ".cnj_close2 {font-size:8pt;color:#ff0000; padding:0px; margin:0px; background-color:#EFEFEF;  border: thin solid #808080;  border-style:solid;cursor:hand;font-weight:bold;height:16px;width:16px;text-align:center;vertical-align:bottom}";
	strCalendar += ".cnj_input {background-color:rgb(240,240,240); border: thin solid; height:16pt;cursor:hand;padding:0px; margin:0px; }";
	strCalendar += ".cnj_input2 {font-size:8pt;color:#808080;padding:0px; margin:0px;  background-color:#EFEFEF;  border: thin solid #808080; border-style:solid;cursor:hand;height:16px;}";
	strCalendar += ".cnj_input3 {font-size:8pt;color:#000000;padding:0px; margin:0px;  background-color:#FFFFFF;  border: thin solid #C00000; border-style:solid;cursor:hand;height:16px;}";
	strCalendar += ".cnj_input4 {font-size:8pt;color:#C00000;padding:0px; margin:0px;  background-color:#FFFFFF; border: thin solid #808080; border-style:solid;cursor:hand;height:16px;}";
	strCalendar += ".cnj_td {border-collapse:collapse;  border: thin solid #a0a0a0;padding:0px; margin:0px; }";
	strCalendar += "</style>";
...
function closeCalendarText() {
	oPopup.hide();
	ctlToPlaceValue.value =	 "약 " + constructDate(dateSelected,monthSelected,yearSelected) + " 입고 예정입니다.";
...
function constructCalendarText() {
	var aNumDays = Array (31,0,31,30,31,30,31,31,30,31,30,31);
	var dateMessage;
	var startDate =	new Date (yearSelected,monthSelected,1);
	var endDate;
	...
	for(datePointer=1; datePointer<=numDaysInMonth; datePointer++) {
		...
		if((datePointer == dateNow) && (monthSelected == monthNow) && (yearSelected == yearNow)) {  // 현재 오늘 날짜
			sHTML += "<b><div style='"+sStyle+"' onclick='javascript:parent.dateSelected="+datePointer+";parent.closeCalendarText();'><font color=#00C000> " + datePointer + "</font> </div></b>";
		} else
		//if((dayPointer % 7 == (startAt * -1)+1) || (dayPointer % 7 == (startAt * -1))) {  // 일요일, 토요일
		if(dayPointer % 7 == ((startAt * -1)+1)) {  // 일요일
			sHTML += "<div style='"+sStyle+"' onclick='javascript:parent.dateSelected="+datePointer + ";parent.closeCalendarText();'> <font color=red>" + datePointer + "</font> </div>";
		} else if(dayPointer % 7 == (startAt * -1)) { // 토요일
			sHTML += "<div style='"+sStyle+"' onclick='javascript:parent.dateSelected="+datePointer + ";parent.closeCalendarText();'> <font color=blue>" + datePointer + "</font> </div>";
		} else {
			sHTML += "<div style='"+sStyle+"' onclick='javascript:parent.dateSelected="+ datePointer + ";parent.closeCalendarText();'>" + datePointer + "</div>";
		}
		sHTML += "";
...
function popUpCalendar(ctl, ctl2, format,fn) {
	var leftpos = 0;
	var toppos = 0;
	
	if(fn)afterFuntion=fn;
	else afterFuntion="";
	
	
	if(ctl2)ctl2 = document.getElementById(ctl2);
...
function popUpCalendarText(ctl, ctl2, format,fn) {
	var leftpos = 0;
	var toppos = 0;
	...
	if(bPageLoaded) {
		...
		aFormat	= dateFormat.split(formatChar);
			...
			cleft =	fixedX==-1 ? ctl.offsetLeft	+ leftpos :	fixedX;
			ctop = fixedY==-1 ?	ctl.offsetTop +	ctl.offsetHeight + toppos :	fixedY;
			constructCalendarText (1, monthSelected, yearSelected);
