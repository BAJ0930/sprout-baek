...
<?
		$sql = "Select *  from 2011_memberInfo";
		if($inKeyword)$sql.=" where Mname like '%" . $inKeyword . "%' ";
		else $sql.=" where Mname = -1";
		...
		while($rs=sql_fetch_array($makerResult))
		{
			...
			echo "	<a href='javascript:fnAddSMSList(\"" . $rs["MHP"] . "\")'>" . $rs["MID"] . " / " . $rs["Mname"] . " [ " . $rs["MHP"] . " ]</a>";
			echo "	</td>";
			echo "</tr>";
		}//-- end Loop
