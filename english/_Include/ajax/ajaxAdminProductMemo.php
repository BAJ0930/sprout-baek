<?
	...
	{				
		...
		$sql = "select a.*,b.Mname from 2011_productMemo as a left join 2011_memberInfo as b on a.APMMID = b.MID where APMPIDX='" . $qPIDX . "'  order by APMregdate desc  ";
		...
		<script>
			function fnSaveMemo()
			{
				txt = jQuery("#inAPMtxt").val();
				
				if(!txt)
				{
					alert("내용을 입력해 주세요.");
					jQuery("#inAPMtxt").focus();
					return;
				}
				...
				param = "stepCode=save&content=" + txt + "&qPIDX=<?=$qPIDX?>&MOIDX=" + midx;
				
				$.ajax({
	 			url:'/_Include/ajax/ajaxAdminProductMemo.php',
	 				type:"POST",
					data : param,
					dataType:"text",
					error:fnErrorAjax,
			...
			function fnEditMemo(idx)
			{
				...
				jQuery("#inAPMtxt").val(jQuery("#txt"+idx).html());
		  ...
		  <tr>
		    <td align="center" valign="top">
		    	...
		    	<table class='tbl' style='width:100%;'>
					...
					?>
		    		<tr>
		    			<td class='memoList'><?=date("Y-m-d H:i:s",$dbAPMregdate)?></td>
		    			<td class='memoList'><?=$dbMname?></td>
		    			<td class='memoList' align=left id='txt<?=$dbIDX?>'><?=$dbAPMmemo?></td>
		  ...
		  <tr>
		  	<td align=center>
		  		...
		  		<table width='100%' class='tbl'>
		  			<tr>
		  				<td style='width:600'>메모 : <input type='text' style='width:350px;' maxlength=150 id='inAPMtxt' onkeydown='if(event.keyCode==13)fnSaveMemo();'>(100자)</td>
				  		<td align=left>
	...
	else if($stepCode=="save")
	{
		...
		if($MOIDX)
		{
			$sql = "update 2011_productMemo set ";
			$sql.= "APMmemo = '" . $content . "' ";
			$sql.= " where IDX='" . $MOIDX . "'";
			sql_query($sql);
		}
		...
		{
			$sql = "insert into 2011_productMemo (APMPIDX,APMMID,APMmemo,APMregdate,APMdeleted) values(";
			$sql.= "'" . $qPIDX . "',";
			$sql.= "'" . $MID . "',";
			$sql.= "'" . $content . "',";
			$sql.= "'" . date(time()) . "',";
	...
	else if($stepCode=="delete")
	{
		...
		$sql = "delete from 2011_productMemo where IDX='" . $MOIDX . "'";
		sql_query($sql);
