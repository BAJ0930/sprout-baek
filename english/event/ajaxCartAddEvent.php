<?
	...
	for($k=0;$k<sizeof($opIDX);$k++)
	{
		...
		$sql = "select a.IDX from 2011_cartInfo as a left join 2011_productInfo as b on a.PIDX=b.IDX where b.PcartWith=2 and a.MID='" . $uid . "' and a.PIDX<>'" . $qIDX[$k] . "' and a.CAstate=" . $inCAstate;
		...
		else
		{
			...
			$sql = "select PcartWith from 2011_productInfo where IDX='" . $qIDX[$k] . "'";
			$result = sql_query($sql);
		...
		//-- 중복상품 검사
		if($opTEXT) $sql = "select * from 2011_cartInfo where MID='" . $uid . "' and PIDX='" . $qIDX[$k] . "' and CAstate=" . $inCAstate;
		else $sql = "select * from 2011_cartInfo where MID='" . $uid . "' and OPIDX='" . $opIDX[$k] . "' and PIDX='" . $qIDX[$k] . "' and CAstate=" . $inCAstate;
